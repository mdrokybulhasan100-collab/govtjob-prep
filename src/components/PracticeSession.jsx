...
export default function PracticeSession() {
  ...
  // Content shown once the answer/options are visible (same for both
  // flashcard-after-flip and direct-MCQ mode).
  function renderRevealedBody() {
    return (
      <>
        {isShort ? (
  ...
  return (
    <section className="view">
      ...
      {isFlashcard && !revealed ? (
        // ---- Tap-anywhere flashcard front: click flips the whole card ----
        <button className="flashcard-tap" onClick={reveal} key={`front-${q.id}`}>
          <div className="omr-qnum">{toBn(index + 1).padStart(2, "0")}</div>
          <p className="omr-question">{q.question_text}</p>
          <span className="flashcard-hint">👆 ট্যাপ করে {isShort ? "উত্তর" : "অপশন"} দেখুন</span>
        </button>
      ) : (
        <div className={`omr-sheet ${isFlashcard ? "flashcard-flip-in" : ""}`} key={`back-${q.id}`}>
          <div className="omr-qnum">{toBn(index + 1).padStart(2, "0")}</div>
          <p className="omr-question">{q.question_text}</p>
          {renderRevealedBody()}
        </div>
