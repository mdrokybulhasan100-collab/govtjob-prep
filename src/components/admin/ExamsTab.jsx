...
export default function ExamsTab({ exams, refreshAll, flash }) {
  ...
  function startEdit(e) {
    setEditingId(e.id);
    setForm({ name: e.name, organization: e.organization || "", year: e.year || "", slug: e.slug });
  }
  ...
  async function saveExam() {
    ...
    if (editingId) {
      ...
      flash("ok", "প্রশ্নপত্র আপডেট হয়েছে");
    } else {
  ...
  return (
    <div>
      ...
      <div className="admin-form">
        ...
        <button className="cta-primary" onClick={saveExam}>{editingId ? "✓ Save করুন" : "+ যোগ করুন"}</button>
          ...
          {exams.map((e) => (
            <tr key={e.id}>
              ...
              <td className="admin-row-actions">
                <button className="cta-small" onClick={() => startEdit(e)}>এডিট</button>
                <button className="cta-danger" onClick={() => deleteExam(e.id)}>মুছুন</button>
              </td>
            </tr>
          ))}
        </tbody>
